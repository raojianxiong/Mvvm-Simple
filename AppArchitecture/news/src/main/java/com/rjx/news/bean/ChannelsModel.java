...
import com.rjx.news.api.NewsChannelsBean;
...
public class ChannelsModel extends MvvmBaseModel<NewsChannelsBean, ArrayList<ChannelsModel.Channel>> {
    ...
    protected void load() {
        AppNetworkApi.getService(NewsApiInterface.class)
                .getNewsChannels()
                .compose(AppNetworkApi.getInstance().applySchedulers(new BaseObserver<NewsChannelsBean>(this,this)));
    }

    @Override
    public void onSuccess(NewsChannelsBean t, boolean isFromCache) {
        ArrayList<Channel> channels = new ArrayList<>();
        for (NewsChannelsBean.ChannelList source:t.showapiResBody.channelList){
            Channel channel = new Channel();
            channel.channelId = source.channelId;
            channel.channelName = source.name;
            channels.add(channel);
        }
        loadSuccess(t,channels,isFromCache);
    }
